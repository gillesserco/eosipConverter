...
class ingester_proba(ingester.Ingester):
        ...
        def extractMetadata(self,met,processInfo):
            ...
            #
            startDate = processInfo.srcProduct.metadata.getMetadataValue(metadata.METADATA_START_DATE_TIME)
            sceneCenter = processInfo.srcProduct.metadata.getMetadataValue(metadata.METADATA_SCENE_CENTER)
            toks = processInfo.srcProduct.metadata.getMetadataValue(metadata.METADATA_FOOTPRINT).split(' ')
            ...
            if 1==1: # from ramona
                if velocity[2] >= 0:
                    processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_ORBIT_DIRECTION, valid_values.ASCENDING)
                    processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_FOOTPRINT,
                                                                    '%s %s %s %s %s %s %s %s %s %s' % (toks[4], toks[5],
                                                                                                       toks[2], toks[3],
                                                                                                       toks[0], toks[1],
                                                                                                       toks[6], toks[7],
                                                                                                       toks[4], toks[5]))
                    ...
                    processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_ORBIT_DIRECTION, valid_values.DESCENDING)
                    processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_FOOTPRINT,
                                                                    '%s %s %s %s %s %s %s %s %s %s' % (toks[0], toks[1],
                                                                                                       toks[6], toks[7],
                                                                                                       toks[4], toks[5],
                                                                                                       toks[2], toks[3],
                                                                                                       toks[0], toks[1]))
            ...
            zenith, azimuth = solar.getSunPosition(float(sceneCenter.split(' ')[0]), float(sceneCenter.split(' ')[1]), d)
            #
            ...
            #else:
            processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_SUN_AZIMUTH, azimuth)
            processInfo.srcProduct.metadata.setMetadataPair(metadata.METADATA_SUN_ELEVATION, zenith)
...
if __name__ == '__main__':
    try:
        if len(sys.argv) > 1:
            ...
            print >>out, "proba conversion report"
            print >>out, report
