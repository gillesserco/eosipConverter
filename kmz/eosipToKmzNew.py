...
#  - dontReverseBrowse: boolean used to tell that the browse image is already oriented well
#
def makeLinearRing(xmlData, footprint, footprint2, descending=True, options=None, aProcessInfo=None, dontReverseBrowse = False):
    #
    if xmlData is None:
        xmlDataInfo = 'None'
    else:
        xmlDataInfo = xmlData[0:100]+' ...'
    addLogToProcessInfo(aProcessInfo, " makeLinearRing ; start of xmlData=%s ...; footprintString=%s; footprint2=%s; descending=%s; options=%s; dontReverseBrowse=%s" % (xmlDataInfo, footprint, footprint2, descending, options, dontReverseBrowse))
    #
    ringNodes=[]
    boundingbox=False
    ringString=None
    ...
    if debug:
        ...
        print " makeLinearRing ; start of xmlData=%s ...; footprintString=%s; footprint2=%s; descending=%s; options=%s; dontReverseBrowse=%s" % (xmlDataInfo, footprint, footprint2, descending, options, dontReverseBrowse)
    try:
