...
if [ $# -gt 0 ];then
	echo "syntax run_polygonToCountry.sh"
	exit
fi
...
echo "COMMAND:java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers.jar ${CONFIGURATION_polygonToCowntry}"
java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers_gis.jar ${CONFIGURATION_polygonToCowntry}
