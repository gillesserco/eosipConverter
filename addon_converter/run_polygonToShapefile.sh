...
if [ $# -gt 0 ];then
	echo "syntax run_polygonToShapeFile.sh"
	exit
fi
...
echo "COMMAND:java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers.jar ${CONFIGURATION_polygonToShape}"
java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers_gis.jar ${CONFIGURATION_polygonToShape}
