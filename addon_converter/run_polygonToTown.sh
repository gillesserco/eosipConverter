...
if [ $# -gt 0 ];then
	echo "syntax run_polygonToTown.sh"
	exit
fi
...
echo "COMMAND:java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers.jar ${CONFIGURATION_polygonToTown}"
java -Dlog4j.debug=true -cp ${CLASSPATH} -jar serviceServer/webServer-with-handlers_gis.jar ${CONFIGURATION_polygonToTown}
